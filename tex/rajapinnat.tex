...
\section{SunSpec Modbus}
  ...
  SunSpecin toteutuksessa on määritelty \gls{CIM} (yleinen tietomalli), jonka avulla eri valmistajien laitteita voidaan yhdistää samaan järjestelmään. \gls{CIM} mahdollistaa laiteriippumattomuuden, sillä siinä on määritelty funktioiden toiminta ja niiden käyttö. Tällöin eri valmistajien laitteita voidaan lukea ja ohjata samoilla käskyillä järjestelmän sisällä.

  SunSpec Modbusin määrittelemä \gls{CIM} on käytössä myös muilla protokollilla, kuten \gls{HTTP}/\gls{XML}:lla ja \gls{OPC}:lla. Koska lähes kaikki tieto SunSpecin standardista perustuu Modbus-protokollaan, muiden tiedonsiirtoprotokollien tarkastelu jätettiin pois. Tiedon puute johtuu mitä luultavimmin Modbusin suuresta suosiosta.
